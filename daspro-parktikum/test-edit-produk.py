...
random_deskripsi = ''.join(random.choice(string.ascii_letters) for _ in range(random.randint(1000,5000)))
random_nama = 'Edit '.join(random.choice(string.ascii_letters) for _ in range(random.randint(10, 20)))
...
def editProduk() :
    try:
        ...
        edit_btn = driver.find_elements(By.ID, "editBtn")
        random_edit_btn = random.choice(edit_btn)
        random_edit_btn.click()
        time.sleep(5)
        ...
        time.sleep(3)
        input_nama_produk.send_keys(random_nama)
        time.sleep(3)

        input_kategori_produk.click()
        time.sleep(3)
        kategori = driver.find_elements(By.XPATH, f'//option[@id="kategori-option"]')
        random_kategori = random.choice(kategori)
        random_kategori.click()
        time.sleep(3)

        input_harga_produk.clear()
        time.sleep(3)
        input_harga_produk.send_keys(random.randint(10000,10000000))
        time.sleep(3)

        input_deskripsi_produk.clear()
        time.sleep(3)
        input_deskripsi_produk.send_keys(random_deskripsi)
        time.sleep(3)

        input_stok_produk.clear()
        time.sleep(3)
        input_stok_produk.send_keys(random.radint(1, 20))
        time.sleep(3)
